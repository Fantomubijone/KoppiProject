...
# OTHER FUNCTION
def save_receipt(content):
    receipt_name = simpledialog.askstring("Save Receipt", "Enter name for receipt:")
    db.save_receipt_text(receipt_name, content)

def confirm_logout(root):
    confirmation = messagebox.askyesno("Logout Confirmation", "Are you sure you want to logout?")
    if confirmation:
        messagebox.showinfo("Logout Successful", "You have been successfully logged out. Thank you for ordering!")
        homepage(root)

def order_again(user):
...
def back_to_buy(root, back = 0):
    ...
    if back != 0:
        back_button.bind("<Button-1>", lambda event: checkouts(root))
    else:
...
def checkouts(root):
    ...
    if not orders:
        ...
        create_button(frame, "Pay now", lambda: payment(root))
        ...
        def payment(root):
            def pay_now():
                ...
                total_due = db.calculate_total_due() 
                if selected_payment in ["VISA", "GCASH"]:
                    ...
                    text_receipt.insert(END, content)
                    ...
                    try:
                        ...
                        if cash_amount >= total_due:
                            change_due = cash_amount - total_due
                            
                            messagebox.showinfo("Payment Success", f"Payment completed successfully!")
            ...
            payment = tk.Label(main_frame, text="PAYMENT", font=("Helvetica", 40, "bold"))
            payment.pack(pady=(10, 20))
            
            receipt_frame = tk.Frame(main_frame)
            receipt_frame.pack(side=tk.LEFT, padx=30,pady=30)


            # SHOWING INITIAL RECEIPT
            text_receipt = scrolledtext.ScrolledText(receipt_frame, width=55, height=30, wrap=tk.WORD)
            text_receipt.pack(pady=10)
            text_receipt.delete("1.0", END)
            text_receipt.insert(END,  db.initial_receipt())

            
            payment_frame = tk.Frame(main_frame)
            payment_frame.pack(side=tk.RIGHT, padx=30,pady=30)

            paymentmethod = tk.Label(payment_frame, text="Choose Payment Method", font=("Helvetica", 18, "bold"))
            paymentmethod.pack(pady=(10, 20))
            
            payment_var = StringVar()
            payment_var.set("VISA")  # Default selection


            visa_radio = Radiobutton(payment_frame, text="VISA", variable=payment_var, value="VISA", 
                                        command=lambda: cash_entry.configure(state='disabled'))
            gcash_radio = Radiobutton(payment_frame, text="GCASH", variable=payment_var, value="GCASH", 
                                        command=lambda: cash_entry.configure(state='disabled'))
            cash_radio = Radiobutton(payment_frame, text="CASH", variable=payment_var, value="CASH",
                                        command=lambda: cash_entry.configure(state='normal'))
            ...
            # PAYMENT FORM
            amount_label = Label(payment_frame,text= "Enter Amount")
            cash_entry = ctk.CTkEntry(payment_frame, placeholder_text="Cash Amount", state='disabled', 
                                      justify = 'center')

            amount_label.pack(pady=(20,0))
            cash_entry.pack()

            create_button(payment_frame,"Pay Now", pay_now)
